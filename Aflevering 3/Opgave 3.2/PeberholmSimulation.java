...
public class PeberholmSimulation {
	 ...
	 * 
	 * @param noOfPlants The number of plants.
	...
	private void initGraphics() {
		...
		JPanel peberholm = new JPanel() {
			public void paintComponent(Graphics gr) {
				...
				gr.fillRect(1, 1, cellsize * (noOfCols + 1), cellsize * (noOfRows + 1));
				gr.setColor(oldCol);
				gr.drawRect(1, 1, cellsize * (noOfCols + 1), cellsize * (noOfRows + 1));
			...
			private void drawPlant(Plant pl, Graphics gr, int cellsize) {
				...
				gr.fillRect(pl.position.x * cellsize, pl.position.y * cellsize, cellsize, cellsize);
	...
	private void createInitialPlants() {
		...
		do {
		} while (!addPlantIfPossible(new Bush(PeberholmConstantsAndUtilities.getLegalRandomPosition())));
		do {
		} while (!addPlantIfPossible(new Moss(PeberholmConstantsAndUtilities.getLegalRandomPosition())));
		do {
		} while (!addPlantIfPossible(new Flower(PeberholmConstantsAndUtilities.getLegalRandomPosition())));
		do {
		} while (!addPlantIfPossible(new Tree(PeberholmConstantsAndUtilities.getLegalRandomPosition())));
	...
	private boolean addPlantIfPossible(Plant pl) {
		if (PeberholmConstantsAndUtilities.positionOK(pl.position)
				&& !occupied[pl.getPosition().y][pl.getPosition().x]) {
			plants.add(pl);
